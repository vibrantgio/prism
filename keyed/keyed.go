...
//	    editors := keyed.Defer(func(_ ItemID) *widget.Editor { return &widget.Editor{} })
...
// Defer creates a Deferred state registry. factory is called at most once per
// unique key.
func Defer[K comparable, V any](factory func(K) V) *Deferred[K, V] {
	return &Deferred[K, V]{
		store:   make(map[K]V),
		factory: factory,
	}
